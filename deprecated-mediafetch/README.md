# 📥 Legacy Media Fetcher (Deprecated)

> [!WARNING]
> **DEPRECATED**: This Python implementation of MediaFetch is legacy and no longer actively maintained.
> It has been superseded by the high-performance Rust + Ratatui version located at [**`../mediafetch/`**](../mediafetch/).

A standalone Python wrapper for `yt-dlp` featuring a full-screen alternate-buffer TUI dashboard, multi-threaded downloads (`aria2c`), clipboard URL auto-pasting, smart destination directory routing, preset profiles, and LRCLIB lyrics tagging.
