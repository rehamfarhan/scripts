...
def create_launcher_script(
    ...
    # Protect user-customized wrapper scripts
    if script_path.is_file():
        try:
            content = script_path.read_text(encoding="utf-8", errors="ignore")
            if "# CUSTOM_WRAPPER" in content or "# MANUAL_OVERRIDE" in content or "# DO NOT OVERWRITE" in content:
                return script_path
        except Exception:
            pass
    ...
    lines = [
        ...
        f"# ICON: {icon}",
        "# GENERATED BY RUN.PY - AUTO-MANAGED",
        "set -e",
        'cd "$(dirname "$0")"',
    ]
...
def render_post_game_recap(
    ...
    # Star rating representation with full ⭐ emoji compatible with Fluent Emoji Color
    rating_labels = {
        5: "Masterpiece",
        4: "Great",
        3: "Good",
    ...
    if rating > 0:
        stars_emojis = "⭐" * rating
        rating_text = rating_labels.get(rating, "")
        stars_formatted = f"{stars_emojis}  {COLOR_GREY}({rating}/5 • {rating_text}){COLOR_RESET}"
    else:
    ...
    platform_str = f"{COLOR_CYAN}Windows (Wine/Proton){COLOR_RESET}" if is_windows else f"{COLOR_GREEN}Linux Native{COLOR_RESET}"
    now_str = datetime.datetime.now().strftime("%a, %b %d • %H:%M")

    term_cols, term_rows = shutil.get_terminal_size((80, 24))
    card_width = min(max(54, term_cols - 10), 88)
    inner_width = card_width - 2
    left_pad = max(0, (term_cols - card_width) // 2)
    p_str = " " * left_pad

    dash_len = inner_width
    top_line = f"{p_str}{COLOR_CYAN}✨ ╭{'─' * dash_len}╮ ✨{COLOR_RESET}"
    divider = f"{p_str}{COLOR_CYAN}   ├{'─' * dash_len}┤   {COLOR_RESET}"
    bottom_line = f"{p_str}{COLOR_CYAN}✨ ╰{'─' * dash_len}╯ ✨{COLOR_RESET}"
    ...
    row_dur = f"{p_str}{COLOR_CYAN}   │{COLOR_RESET}{pad_to(f'{pad_to(lbl1, label_w)}{val1}', inner_width)}{COLOR_CYAN}│   {COLOR_RESET}"

    val2 = f"{COLOR_CYAN}{tot_str}{COLOR_RESET}   {COLOR_GREY}(Session #{play_count}){COLOR_RESET}"
    row_tot = f"{p_str}{COLOR_CYAN}   │{COLOR_RESET}{pad_to(f'{pad_to(lbl2, label_w)}{val2}', inner_width)}{COLOR_CYAN}│   {COLOR_RESET}"

    row_rat = f"{p_str}{COLOR_CYAN}   │{COLOR_RESET}{pad_to(f'{pad_to(lbl3, label_w)}{stars_formatted}', inner_width)}{COLOR_CYAN}│   {COLOR_RESET}"

    val4 = f"{COLOR_GREY}{now_str}{COLOR_RESET}"
    row_date = f"{p_str}{COLOR_CYAN}   │{COLOR_RESET}{pad_to(f'{pad_to(lbl4, label_w)}{val4}', inner_width)}{COLOR_CYAN}│   {COLOR_RESET}"

    row_plat = f"{p_str}{COLOR_CYAN}   │{COLOR_RESET}{pad_to(f'{pad_to(lbl5, label_w)}{platform_str}', inner_width)}{COLOR_CYAN}│   {COLOR_RESET}"
...
def launch_game_attached(games_root: Path, folder_key: str, script_path: Path) -> None:
    ...
    try:
        ...
        print("\nInterrupted by user. Terminating game...")
        if proc:
            try:
                proc.terminate()
                proc.wait(timeout=2)
    ...
    render_post_game_recap(
        ...
        is_windows=game_info.get("is_windows", False),
    )
    sys.exit(0)


# --- Execution & Video Driver Settings Wizard ---
...
def configure_execution_settings(
    ...
    """Prompt interactively for execution settings (Wine prefix/resolution for Windows, SDL video driver for Linux)."""
    if not is_windows:
        sdl_options = [
            "1) Auto / System Default (Recommended)",
            "2) Force X11 (Xwayland compatibility mode - e.g. for Hacknet)",
            "3) Force Wayland",
        ]
        clear_screen()
        ans_sdl, _, _ = run_fzf(
            sdl_options,
            prompt="SDL Video Driver: ",
            border_label=f"🐧 Linux Video Driver ({current_name})",
        )
        sdl_driver = None
        if ans_sdl:
            if "Force X11" in ans_sdl:
                sdl_driver = "x11"
            elif "Force Wayland" in ans_sdl:
                sdl_driver = "wayland"
        return None, {"sdl_driver": sdl_driver}
    ...
    sdl_options_win = [
        "1) Auto / System Default (Recommended)",
        "2) Force X11 (Xwayland)",
        "3) Force Wayland",
    ]
    clear_screen()
    ans_sdl_win, _, _ = run_fzf(
        sdl_options_win,
        prompt="SDL Video Driver (Wine): ",
        border_label=f"🍷 Video Driver ({current_name})",
    )
    sdl_driver_win = None
    if ans_sdl_win:
        if "Force X11" in ans_sdl_win:
            sdl_driver_win = "x11"
        elif "Force Wayland" in ans_sdl_win:
            sdl_driver_win = "wayland"

    opts = {
        "resolution": resolution,
        "dxvk_hud": dxvk_hud,
        "win_ver": win_ver,
        "sdl_driver": sdl_driver_win,
    }
...
def configure_new_folders(games_root: Path, registry: Dict[str, Any], stats_data: Dict[str, Any]) -> bool:
    ...
    for idx, folder in enumerate(unregistered, start=1):
        ...
        wine_prefix, wine_opts = configure_execution_settings(games_root, folder, is_windows, current_name=display_name)
...
def show_utilities_menu(
    ...
    if "Rename Game Entry" in ans:
    ...
    elif "Execution & Video Driver" in ans or "Configure Wine" in ans:
        folder_path = games_root / folder_key
        is_win = game_info.get("is_windows", False)
        prefix, wine_opts = configure_execution_settings(games_root, folder_path, is_win, current_name=g_name)
    ...
    elif "Edit Launcher Script" in ans:
        folder_path = games_root / folder_key
        ...
        game_info["custom_wrapper"] = True
        save_registry(games_root, registry)
        clear_screen()
        subprocess.run([editor, str(script_path.resolve())])
        return f"{COLOR_GREEN}✓ Saved custom wrapper for '{g_name}'{COLOR_RESET}"
        ...
        # Preserve stats_data in .run_stats.json permanently
        save_registry(games_root, registry)
        return f"{COLOR_RED}🗑️ Removed '{g_name}' launcher entry (stats preserved){COLOR_RESET}"
...
def quick_launch(games_root: Path, query: str, registry: Dict[str, Any]) -> None:
    ...
    # Always ensure start_game.sh is up to date with SDL_VIDEODRIVER=x11
    script_path = create_launcher_script(
        folder_path,
        Path(target["binary"]),
        target["display_name"],
    ...
    launch_game_attached(games_root, target["dir"], script_path)
...
def main_launcher_menu(games_root: Path, registry: Dict[str, Any], stats_data: Dict[str, Any]) -> None:
    ...
    while True:
        ...
        launch_game_attached(games_root, folder_key, script_path)
        break


def repair_all_launchers(games_root: Path, registry: Dict[str, Any]) -> None:
    """Regenerate all launcher scripts on disk to ensure clean driver and execution settings."""
    games = registry.get("games", {})
    for folder_key, game_info in games.items():
        folder_path = games_root / folder_key
        if not folder_path.is_dir():
            continue
        binary_rel = game_info.get("binary")
        if not binary_rel:
            continue
        display_name = game_info.get("display_name", folder_key)
        is_win = game_info.get("is_windows", False)
        prefix = Path(game_info["wine_prefix"]) if game_info.get("wine_prefix") else None
        opts = game_info.get("wine_opts", {})
        try:
...
def main() -> None:
    ...
    # Heal all existing launchers on disk
    repair_all_launchers(games_root, registry)
